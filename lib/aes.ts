class AES {
  private sBox: number[];
  private invSBox: number[];
  private rCon: number[];
  private key: number[];
  private roundKeys: number[];

  constructor(key: number[]) {
    if (key.length !== 16) {
      throw new Error("Key must be 128 bits (16 bytes) long");
    }

    this.sBox = [
      ,
    ];

    this.invSBox = [
      ,
    ];

    this.rCon = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36];
    this.key = key;
    this.roundKeys = this.keyExpansion(key);
  }

  private subBytes(state: number[][]): void {
    for (let i = 0; i < 4; i++) {
      for (let j = 0; j < 4; j++) {
        state[i][j] = this.sBox[state[i][j]];
      }
    }
  }

  private shiftRows(state: number[][]): void {
    for (let i = 1; i < 4; i++) {
      state[i] = [...state[i].slice(i), ...state[i].slice(0, i)];
    }
  }

  private mixColumns(state: number[][]): void {
    for (let i = 0; i < 4; i++) {
      const s0 = state[0][i];
      const s1 = state[1][i];
      const s2 = state[2][i];
      const s3 = state[3][i];

      state[0][i] = (this.mul(0x02, s0) ^ this.mul(0x03, s1) ^ s2 ^ s3) & 0xff;
      state[1][i] = (s0 ^ this.mul(0x02, s1) ^ this.mul(0x03, s2) ^ s3) & 0xff;
      state[2][i] = (s0 ^ s1 ^ this.mul(0x02, s2) ^ this.mul(0x03, s3)) & 0xff;
      state[3][i] = (this.mul(0x03, s0) ^ s1 ^ s2 ^ this.mul(0x02, s3)) & 0xff;
    }
  }

  private addRoundKey(state: number[][], roundKey: number[]): void {
    for (let i = 0; i < 4; i++) {
      for (let j = 0; j < 4; j++) {
        state[i][j] ^= roundKey[i + 4 * j];
      }
    }
  }

  private keyExpansion(key: number[]): number[] {
    const expandedKeys: number[] = [...key];

    for (let i = 4; i < 44; i++) {
      let temp = expandedKeys.slice((i - 1) * 4, i * 4);
      if (i % 4 === 0) {
        temp = this.keyExpansionCore(temp, i / 4);
      }
      for (let j = 0; j < 4; j++) {
        expandedKeys.push(expandedKeys[(i - 4) * 4 + j] ^ temp[j]);
      }
    }

    return expandedKeys;
  }

  private keyExpansionCore(word: number[], iteration: number): number[] {
    const result = [...word];
    const temp = result.shift()!;
    result.push(temp);

    for (let i = 0; i < 4; i++) {
      result[i] = this.sBox[result[i]];
    }

    result[0] ^= this.rCon[iteration - 1];
    return result;
  }

  encrypt(plaintext: number[]): { cipherText: number[]; steps: string[] } {
    if (plaintext.length !== 16) {
      throw new Error("Plaintext must be 128 bits (16 bytes) long");
    }

    let state = this.createStateMatrix(plaintext);
    let steps: string[] = [`Initial state:\n${this.stateToString(state)}`];

    this.addRoundKey(state, this.roundKeys.slice(0, 16));
    steps.push(`After initial AddRoundKey:\n${this.stateToString(state)}`);

    for (let round = 1; round < 10; round++) {
      this.subBytes(state);
      steps.push(
        `Round ${round} - After SubBytes:\n${this.stateToString(state)}`
      );

      this.shiftRows(state);
      steps.push(
        `Round ${round} - After ShiftRows:\n${this.stateToString(state)}`
      );

      this.mixColumns(state);
      steps.push(
        `Round ${round} - After MixColumns:\n${this.stateToString(state)}`
      );

      this.addRoundKey(
        state,
        this.roundKeys.slice(round * 16, (round + 1) * 16)
      );
      steps.push(
        `Round ${round} - After AddRoundKey:\n${this.stateToString(state)}`
      );
    }

    this.subBytes(state);
    steps.push(`Final round - After SubBytes:\n${this.stateToString(state)}`);

    this.shiftRows(state);
    steps.push(`Final round - After ShiftRows:\n${this.stateToString(state)}`);

    this.addRoundKey(state, this.roundKeys.slice(160));
    steps.push(
      `Final round - After AddRoundKey:\n${this.stateToString(state)}`
    );

    const cipherText = this.stateToCipherText(state);
    steps.push(`Ciphertext:\n${this.byteArrayToHexString(cipherText)}`);

    return { cipherText, steps };
  }

  decrypt(ciphertext: number[]): { plainText: number[]; steps: string[] } {
    if (ciphertext.length !== 16) {
      throw new Error("Ciphertext must be 128 bits (16 bytes) long");
    }

    let state = this.cipherTextToState(ciphertext);
    let steps: string[] = [`Initial state:\n${this.stateToString(state)}`];

    this.addRoundKey(state, this.roundKeys.slice(160));
    steps.push(`After initial AddRoundKey:\n${this.stateToString(state)}`);

    for (let round = 9; round > 0; round--) {
      this.invShiftRows(state);
      steps.push(
        `Round ${10 - round} - After InvShiftRows:\n${this.stateToString(
          state
        )}`
      );

      this.invSubBytes(state);
      steps.push(
        `Round ${10 - round} - After InvSubBytes:\n${this.stateToString(state)}`
      );

      this.addRoundKey(
        state,
        this.roundKeys.slice(round * 16, (round + 1) * 16)
      );
      steps.push(
        `Round ${10 - round} - After AddRoundKey:\n${this.stateToString(state)}`
      );

      this.invMixColumns(state);
      steps.push(
        `Round ${10 - round} - After InvMixColumns:\n${this.stateToString(
          state
        )}`
      );
    }

    this.invShiftRows(state);
    steps.push(
      `Final round - After InvShiftRows:\n${this.stateToString(state)}`
    );

    this.invSubBytes(state);
    steps.push(
      `Final round - After InvSubBytes:\n${this.stateToString(state)}`
    );

    this.addRoundKey(state, this.roundKeys.slice(0, 16));
    steps.push(
      `Final round - After AddRoundKey:\n${this.stateToString(state)}`
    );

    const plainText = this.stateToCipherText(state);
    steps.push(`Plaintext:\n${this.byteArrayToHexString(plainText)}`);

    return { plainText, steps };
  }

  private createStateMatrix(input: number[]): number[][] {
    const state: number[][] = [];
    for (let i = 0; i < 4; i++) {
      state[i] = [];
      for (let j = 0; j < 4; j++) {
        state[i][j] = input[i + 4 * j];
      }
    }
    return state;
  }

  private stateToCipherText(state: number[][]): number[] {
    const cipherText: number[] = [];
    for (let col = 0; col < 4; col++) {
      for (let row = 0; row < 4; row++) {
        cipherText.push(state[row][col]);
      }
    }
    return cipherText;
  }

  private cipherTextToState(cipherText: number[]): number[][] {
    const state: number[][] = [];
    for (let row = 0; row < 4; row++) {
      state[row] = [];
      for (let col = 0; col < 4; col++) {
        state[row][col] = cipherText[col * 4 + row];
      }
    }
    return state;
  }

  private invShiftRows(state: number[][]): void {
    for (let i = 1; i < 4; i++) {
      state[i] = [...state[i].slice(-i), ...state[i].slice(0, -i)];
    }
  }

  private invSubBytes(state: number[][]): void {
    for (let i = 0; i < 4; i++) {
      for (let j = 0; j < 4; j++) {
        state[i][j] = this.invSBox[state[i][j]];
      }
    }
  }

  private invMixColumns(state: number[][]): void {
    for (let i = 0; i < 4; i++) {
      const s0 = state[0][i];
      const s1 = state[1][i];
      const s2 = state[2][i];
      const s3 = state[3][i];

      state[0][i] =
        (this.mul(0x0e, s0) ^
          this.mul(0x0b, s1) ^
          this.mul(0x0d, s2) ^
          this.mul(0x09, s3)) &
        0xff;
      state[1][i] =
        (this.mul(0x09, s0) ^
          this.mul(0x0e, s1) ^
          this.mul(0x0b, s2) ^
          this.mul(0x0d, s3)) &
        0xff;
      state[2][i] =
        (this.mul(0x0d, s0) ^
          this.mul(0x09, s1) ^
          this.mul(0x0e, s2) ^
          this.mul(0x0b, s3)) &
        0xff;
      state[3][i] =
        (this.mul(0x0b, s0) ^
          this.mul(0x0d, s1) ^
          this.mul(0x09, s2) ^
          this.mul(0x0e, s3)) &
        0xff;
    }
  }

  private mul(a: number, b: number): number {
    let result = 0;
    let hiBitSet;
    for (let i = 0; i < 8; i++) {
      if (b & 1) {
        result ^= a;
      }
      hiBitSet = a & 0x80;
      a <<= 1;
      if (hiBitSet) {
        a ^= 0x1b;
      }
      b >>= 1;
    }
    return result;
  }

  private stateToString(state: number[][]): string {
    return state
      .map((row) =>
        row.map((byte) => byte.toString(16).padStart(2, "0")).join(" ")
      )
      .join("\n");
  }

  private byteArrayToHexString(byteArray: number[]): string {
    return byteArray.map((byte) => byte.toString(16).padStart(2, "0")).join("");
  }
}

export default AES;
